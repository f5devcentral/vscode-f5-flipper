...
export class RegExTree {
    ...
    private regexTree: AdcRegExTree = {
        ...
        parents: {
            ...
            'add lb vserver': /add lb vserver (?<name>\S+) (?<protocol>\S+) (?<ipAddress>[\d.]+) (?<port>(\d+|\*)) (?<opts>[\S ]+)/,
            'add lb monitor': /add lb monitor (?<name>\S+) (?<protocol>\S+) (?<opts>[\S ]+)/,
            'add ssl certKey': /add ssl certKey (?<name>\S+) (?<opts>[\S ]+)/,
            'add server': /add server (?<name>\S+) (?<value>\S+)/,
            'add service': /add service (?<name>\S+) (?<value>\S+)/,
            'add serviceGroup': /add serviceGroup (?<name>\S+) (?<protocol>\S+) (?<opts>[\S ]+)/,
            'add cs vserver': /add cs vserver (?<name>\S+) (?<protocol>\S+) (?<ipAddress>[\d.]+) (?<port>(\d+|\*)) (?<opts>[\S ]+)/,
            'add cs action': /add cs action (?<name>\S+) (?<opts>[\S ]+)/,
            'add cs policy': /add cs policy (?<name>\S+) (?<opts>[\S ]+)/,
            'add gslb vserver': /(?<name>\S+) (?<protocol>\S+) (?<opts>[\S ]+)/,
            ...
            'bind serviceGroup': /bind serviceGroup (?<name>\S+) ((?<serv>\S+ \d+)|(?<monitor>-monitorName \S+)|(?<opts>[\S ]+))/,
